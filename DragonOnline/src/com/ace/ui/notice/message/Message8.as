package com.ace.ui.notice.message {
	...
	public class Message8 extends Sprite {
		...
		public function showNext():void {
			...
			var ninfo:TNoticeInfo=noticeInfo.shift();

			var bool:Boolean=false;
			var render:NoticeIcon;
			var content:String=ninfo.content;
			var flag:int=content.indexOf("|");
			var type:int=int(content.substring(0, flag));

			if (14 == type) {
				for (var n:int=0; n < renders.length; n++) {
					render=renders[n];
					if (render.id == ninfo.id) {
						this.renders.splice(n, 1);
			...
			render=NoticeManager.getInstance().noticeIconReuseDic.getFreeRender() as NoticeIcon;
			render.addEventListener(MouseEvent.CLICK, onMouseClick);
			render.updateInfo(ninfo, noticeData.shift());
			addChild(render);
			renders.push(render);
