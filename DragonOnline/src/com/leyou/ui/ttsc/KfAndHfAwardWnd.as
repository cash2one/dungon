package com.leyou.ui.ttsc {
	...
	public class KfAndHfAwardWnd extends AutoWindow {
		...
		public function KfAndHfAwardWnd() {
...
//			}
			...
			var parr:Array=TableManager.getInstance().getPayPromotionByType(type, this.minfo.st + 5);
		...
		public function updateInfo(o:Object):void {
			...
			var i:int=0;
			var count:int=0;
			var data:PayPromotionDataII=DataManager.getInstance().payPromotionData_IIII;
...
//			}
			...
			if (dArr.length == 0) {
				UIManager.getInstance().rightTopWnd.setEffect("todayAwardBtn", false);
				UIManager.getInstance().rightTopWnd.deactive("todayAwardBtn");
				return;
			} else {
				UIManager.getInstance().rightTopWnd.setEffect("todayAwardBtn", true);
				UIManager.getInstance().rightTopWnd.active("todayAwardBtn");
			}
...
//			if (this.minfo.st == 1) {
			this.titleIcon.updateBmp("ui/wybq/font_mrsl.png");
//			} else if (this.minfo.st == 2) {
...
//					btnWidget.y=0;
		...
		private function turnTo(type:int):void {
			...
			var payarr:Array=TableManager.getInstance().getPayPromotionByType(type, this.minfo.st + 5);
			payarr.sortOn("id", Array.CASEINSENSITIVE | Array.NUMERIC);
...
//			timeLbl.htmlText=tinfo.des2;
			...
			var dv:Object=DataManager.getInstance().payPromotionData_IIII.getMinV(type);
