...
//			yb1ProLbl = getUIbyID("yb1ProLbl") as Label;
			yb2ProLbl = getUIbyID("yb2ProLbl") as Label;
...
//			yb1ProLbl.addEventListener(MouseEvent.MOUSE_OVER, onMouseOver);
			yb2ProLbl.addEventListener(MouseEvent.MOUSE_OVER, onMouseOver);
...
//			Cmd_LDW.cm_LDW_H(2);
		...
		protected function onBtnClick(event:MouseEvent):void{
			...
			switch(event.target.name){
				...
				case "ybLotteryBtn":
					if(!isLoop){
						content = TableManager.getInstance().getSystemNotice(6104).content;
		...
		public function updateInfo_I():void{
			var data:LuckDrawData = DataManager.getInstance().luckdrawData;
			pRemainLbl.text = "今日剩余次数" + data.remainCount + "/" + data.maxCount;
		...
		public function generateLog(info:LuckDrawLogInfo):String{
			...
			var str:String = "";
			if(1 == info.type){
				str = "<font color='#FFC000'>{1}</font>获得<font color='{2}'><u><a href='event:{3}|{4}'>{5}</a></u></font>";
				str = StringUtil.substitute(str, info.dtime, color, "item", info.itemid, itemName);
			}else if(2 == info.type){
				str = "<font color='#00ff00'><u><a href='event:{1}|{2}'>{3}</a></u></font>获得了<font color='{4}'><u><a href='event:{5}|{6}'>{7}</a></u></font>";
