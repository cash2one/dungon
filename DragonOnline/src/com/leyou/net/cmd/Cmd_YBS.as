...
//					content=PropUtils.getStringById(1570) + "</a></u></font>";
					...
					arr=[PropUtils.getStringById(2423), "<a href='event:other_ycp--" + monsterInfo.name + "'>" + PropUtils.getStringById(2439) + "</a>", content, "", callBack, "", callBack];
...
//					content=StringUtil.substitute(content, monsterInfo.name, bossId);
					arr=[PropUtils.getStringById(2423), "<a href='event:other_ycp--" + monsterInfo.name + "'>" + PropUtils.getStringById(2439) + "</a>", PropUtils.getStringById(2454), "", "", callBack, "", callBack];
					UIManager.getInstance().taskTrack.updateOtherTrack(TaskEnum.taskLevel_fieldbossCopyLine, arr);
				}
			}
		}
...
//			trace(obj)
			...
			if (count > 0) {
				...
				arr=[PropUtils.getStringById(2423), "<a href='event:other_ycp--ycps'>" + PropUtils.getStringById(2438) + "</a>", content, "", callBack1, "", callBack1];
				UIManager.getInstance().taskTrack.updateOtherTrack(TaskEnum.taskLevel_bosStandLine, arr);
			} else {
				arr=[PropUtils.getStringById(2423), "<a href='event:other_ycp--ycps'>" + PropUtils.getStringById(2438) + "</a>", PropUtils.getStringById(2454), "", callBack1, "", callBack1];
				UIManager.getInstance().taskTrack.updateOtherTrack(TaskEnum.taskLevel_bosStandLine, arr);
			}
