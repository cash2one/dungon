...
//			des2Lbl.htmlText = TableManager.getInstance().getSystemNotice(5322).content;
		...
		protected function onBtnClick(event:MouseEvent):void{
			...
			switch(event.target.name){
				case "cflagBtn":
					content = StringUtil.substitute(content, "财付通", ConfigEnum.invest3, ConfigEnum.invest4, ConfigEnum.invest1);
					PopupManager.showConfirm(content, buyCFT, null, false, "tzlc.buy");
					break;
				case "yflagBtn":
					content = StringUtil.substitute(content, "余额宝", ConfigEnum.invest6, ConfigEnum.invest7, ConfigEnum.invest1);
					PopupManager.showConfirm(content, buyYEB, null, false, "tzlc.buy");
					break;
				case "lflagBtn":
					content = StringUtil.substitute(content, "零钱宝", ConfigEnum.invest9, ConfigEnum.invest10, ConfigEnum.invest1);
		...
		public function updateLog(data:InvestData):void{
			...
			for(var n:int = 0; n < length; n++){
				...
				var content:String = "<font face='SimSun' color='#ffea00' size='12'><u><a href='event:{1}'>{2}</a></u></font><font face='SimSun' color='#CDB97C' size='12'>在{3}领取绑定钻石:{4}</Font>";
