...
//			}
			...
			sumPrice.text=price + "";
			proCount=obj.num;
			autoCKBox.text=StringUtil.substitute(PropUtils.getStringById(1519), [proCount]);
...
//				des+="您在" + obj.mn + "地图被" + StringUtil_II.getColorStr(StringUtil_II.addEventString(obj.name, "[" + obj.name + "]", true), ChatEnum.COLOR_USER) + "击败了";
			...
			if (!isMonster && hasProtected()) {
				des+=PropUtils.getStringById(1520);
			}
			desLbl.htmlText=des;
			startCount();
		}
		...
		protected function onEnterFrame(event:Event):void {
			...
			aqBtn.text=PropUtils.getStringById(2141) + "(" + int(interval / 1000) + ")";
			if (interval <= 0) {
