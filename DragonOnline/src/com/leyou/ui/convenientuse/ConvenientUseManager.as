	...
	public class ConvenientUseManager {
		...
		public function checkNewBagId(id:String):void {
			...
			if (1 == bagInfo.info.classid) {
				// 装备
				if (currentItem.uid == id || int(bagInfo.aid) == 6015) { // 是否是当前显示
					return;
				}
				checkEquip(bagInfo);
...
//			if (int(id) >= 41000 && int(id) <= 43999) {
//				if ((-1 == itemQueue.indexOf(info.tips.uid)) && (Core.me.info.level >= info.info.level)) {
//					itemQueue.push(info.tips.uid);
//				}
//			}

			if (info != null && TItemInfo(info.info).quickUse == "1") {
				if ((-1 == itemQueue.indexOf(info.tips.uid)) && (Core.me.info.level >= info.info.level)) {
					itemQueue.push(info.tips.uid);
				}
			}
...
//					equipQueue[n] = null;
		...
		private function isShow():Boolean {
			return UIManager.getInstance().convenientWear.visible || UIManager.getInstance().convenientTransfer.visible || UIManager.getInstance().convenientUse.visible;
		}
