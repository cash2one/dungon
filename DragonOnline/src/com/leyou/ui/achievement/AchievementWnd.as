package com.leyou.ui.achievement {
	...
	public class AchievementWnd extends AutoWindow {
		...
		private var btnArr:Array=["ZLBtn", "DJBtn", "RWBtn", "ZBBtn", "YSBtn", "XZBtn", "BSBtn", "HHBtn", "FBBtn", "JJBtn", "YWBtn", "CZBtn"];

		private function initCJBtns():void {
			var btn:ImgLabelButton;
			for (var i:int=0; i < this.btnArr.length; i++) {
				btn=getUIbyID(this.btnArr[i]) as ImgLabelButton;
...
//		}
		...
		protected function onButtonClick(event:MouseEvent):void {
			...
			if (this.btnArr.indexOf(event.target.name) != -1) {
				exchangeToEra(true);
				showEraItemsByType(this.btnArr.indexOf(event.target.name) + 1);
				currentBtn=event.target as ImgLabelButton;
			} else {
