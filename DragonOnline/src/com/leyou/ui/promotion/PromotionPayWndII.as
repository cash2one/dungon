	...
	public class PromotionPayWndII extends AutoWindow
	{
		...
		private function refreshBtn(data:PayPromotionDataII):void{
			...
			if(!isContains && (tbuttons.length > 0)){
				currentType = int(tbuttons[m].name.substr(4));
			}
		}
		...
		private function turnTo(type:int):void {
			...
			var dv:int = DataManager.getInstance().payPromotionData_II.getMinV(type);
		...
		private function clearPanel(showType:int):void{
			var index:int;
			var length:int;
			if(1 == showType){
				index = 0;
