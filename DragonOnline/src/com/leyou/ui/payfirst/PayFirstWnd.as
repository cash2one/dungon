...
//			}
		...
		private function initPayInfo():void{
			...
			for each(var str:String in payList){
				...
				if("-1" == data[1]){
					payStr += StringUtil.substitute("{1}钻石以上\n", data[0]);
				}else{
					payStr += StringUtil.substitute("{1}-{2}钻石\n", data[0], data[1]);
				}
				retutrnStr += StringUtil.substitute("额外获得{1}%钻石\n", data[2]);
			}
			payLbl.text = payStr;
		...
		public function updateInfo(obj:Object):void{
			...
			for each(var str:String in payList){
				...
				if((cz >= int(data[0])) && (("-1" == data[1]) || (cz <= int(data[1])))){
					...
					returnRateLbl.text = StringUtil.substitute("{1}%钻石返现", data[2]);
				}
			}
