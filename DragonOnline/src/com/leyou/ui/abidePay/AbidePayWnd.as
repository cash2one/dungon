...
//			lv3Lbl = getUIbyID("lv3Lbl") as Label;
			...
			dayValue1Lbl.text = StringUtil.substitute("充值{1}钻石", tData1.ib);
...
//			lv1Lbl.text = tData1.ib+"";
			...
			dayValue2Lbl.text = StringUtil.substitute("充值{1}钻石", tData2.ib);
...
//			lv2Lbl.text = tData2.ib+"";
			...
			dayValue3Lbl.text = StringUtil.substitute("充值{1}钻石", tData3.ib);
...
//			lv3Lbl.text = tData3.ib+"";
		...
		public function updateInfo():void{
			...
			var begin:String = DateUtil.formatDate(new Date((data.stime)*1000), "YYYY年MM月DD日");
			var end:String = DateUtil.formatDate(new Date((data.etime-1)*1000), "YYYY年MM月DD日");
			timeLbl.text = StringUtil.substitute("活动时间：{1}-{2}", begin, end);
			...
			if(-1 != data.receiveTypeArr.indexOf(tData1.ib+"")){
				...
				if(data.cpayValue < tData1.ib){
					receive1Btn.text = "未达成";
					receive1Btn.setActive(false, 1, true);
				}else{
					receive1Btn.text = "领取奖励";
			...
			if(-1 != data.receiveTypeArr.indexOf(tData2.ib+"")){
				...
				if(data.cpayValue < tData2.ib){
					receive2Btn.text = "未达成";
					receive2Btn.setActive(false, 1, true);
				}else{
					receive2Btn.text = "领取奖励";
			...
			content = StringUtil.substitute("已完成{1}天充值", day);
			...
			rateLbl.text = data.cpayValue+"钻石";
			lineImg.y = emask.y;
