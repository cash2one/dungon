package com.leyou.ui.welfare.child.page {
	...
	public class WelfareTimePage extends AutoSprite {
		...
		private var expRImg:Image;
		private var moneyRImg:Image;
		private var energyRImg:Image;
		private var marketRImg:Image;
		private var bybRImg:Image;
		...
		private function init():void {
			expRImg = getUIbyID("expRImg") as Image;
			moneyRImg = getUIbyID("moneyRImg") as Image;
			energyRImg = getUIbyID("energyRImg") as Image;
			marketRImg = getUIbyID("marketRImg") as Image;
			bybRImg = getUIbyID("bybRImg") as Image;
			mouseChildren=true;
			...
			swapChildren(expRollWidget, expRImg);
			...
			moneyRollWidget.y=263;
			moneyRollWidget.isPopNum=false;
			addChild(moneyRollWidget);
			swapChildren(moneyRollWidget, moneyRImg);
			...
			energyRollWidget.y=156;
			energyRollWidget.isPopNum=false;
			addChild(energyRollWidget);
			swapChildren(energyRollWidget, energyRImg);
			...
			marketGrid.y=351;
			addChild(marketGrid);
			swapChildren(marketGrid, marketRImg);
			...
			swapChildren(ybRollWidget, bybRImg);
			ybRollWidget.alignRound();
		...
		protected function updateTime():void {
			if (updateExp) {
				if ("" != getTimeString(expRemain, expTick)) {
					expTimeLbl.text=PropUtils.getStringById(1979) + getTimeString(expRemain, expTick) + PropUtils.getStringById(1980);
				} else {
					expTimeLbl.text=PropUtils.getStringById(1980);
			...
			if (updateMoney) {
				if ("" != getTimeString(moneyRemain, moneyTick)) {
					moneyTimeLbl.text=PropUtils.getStringById(1979) + getTimeString(moneyRemain, moneyTick) + PropUtils.getStringById(1980);
				} else {
					moneyTimeLbl.text=PropUtils.getStringById(1980);
			...
			if (updateEnergy) {
				if ("" != getTimeString(energyRemain, energyTick)) {
					energyTimeLbl.text=PropUtils.getStringById(1979) + getTimeString(energyRemain, energyTick) + PropUtils.getStringById(1980);
				} else {
					energyTimeLbl.text=PropUtils.getStringById(1980);
			...
			if (updateMarket) {
				if ("" != getTimeString(marketRemain, marketTick)) {
					marketTimeLbl.text=PropUtils.getStringById(1979) + getTimeString(marketRemain, marketTick) + PropUtils.getStringById(1980);
				} else {
					marketTimeLbl.text=PropUtils.getStringById(1980);
...
//			trace("------------------------ type = "+type+"--btnVaild = " + btnV);
			switch (type) {
				case 1:
					...
					expLotteryBtn.text=receive ? PropUtils.getStringById(1980) : PropUtils.getStringById(1981);
					//					receive = (0 >= stime);
					expLotteryBtn.setActive(btnV, 1, true);
					expRImg.visible = !receive;
					break;
				case 2:
					updateMoney=receive;
					moneyRemain=stime;
					...
					if (value <= 0) {
						moneyRollWidget.setNum(99999);
					}
					moneyTimeLbl.visible=receive;
					moneyLotteryBtn.text=receive ?PropUtils.getStringById(1980) : PropUtils.getStringById(1981);
					//					receive = (0 >= stime);
					moneyLotteryBtn.setActive(btnV, 1, true);
					moneyRImg.visible = !receive;
					break;
				case 3:
					...
					energyLotteryBtn.text=receive ? PropUtils.getStringById(1980) : PropUtils.getStringById(1981);
					//					receive = (0 >= stime);
					energyLotteryBtn.setActive(btnV, 1, true);
					energyRImg.visible = !receive;
					break;
				case 4:
					...
					marketLotteryBtn.text=receive ? PropUtils.getStringById(1980) : PropUtils.getStringById(1981);
					//					receive = (0 >= stime);
					marketLotteryBtn.setActive(btnV, 1, true);
					marketRImg.visible = !receive;
					break;
				case 5:
					updateYb=receive;
					ybRemain=stime;
