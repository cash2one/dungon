package com.leyou.ui.guild.child {
	...
	public class GuildSkill extends AutoSprite {
		...
		private var itemRender:Vector.<GuildSkillRender>;
		private var infoData:Array=[];

		private var currentAtt:int=-1;
		private var upgrade:Boolean=false;

		public function GuildSkill() {
			super(LibManager.getInstance().getXML("config/ui/guild/guildSkill.xml"));
			this.init();
		...
		private function init():void {
			...
			this.itemRender=new Vector.<GuildSkillRender>();
		...
		private function onClick(e:MouseEvent):void {
			...
			if (info == null || UIManager.getInstance().guildWnd.guildLv < info.uLv) {
		...
		private function onItemClick(e:MouseEvent):void {
			...
			var render:GuildSkillRender;
			for each (render in this.itemRender) {
				render.state=false;
			}

			render=e.currentTarget as GuildSkillRender;
